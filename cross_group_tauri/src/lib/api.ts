...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new ApiError("��˷���δ���ӣ���ȷ�ϱ��ط���������", "network");
  }
  ...
  try {
    ...
    if (!res.ok) {
      throw new ApiError(`�����Ӧ�쳣 (${res.status})`, "backend");
    }
  }

  if (!res.ok) {
    throw new ApiError(String(data.error ?? `����ʧ��: ${res.status}`), "backend");
  }
  return data as T;
}
...
export function validateHealthPayload(data: HealthResponse): HealthResponse {
  if (!data.ok) {
    throw new ApiError("��˷����쳣", "backend");
  }
  if (data.service !== SERVICE_ID) {
    throw new ApiError(
      `17888 �˿ڱ�ռ�ã�service=${data.service ?? "unknown"}��`,
      "port_conflict",
    );
  }
  return data;
